...
namespace RomanDateTime
{
    public partial struct RomanDateTime
    {
        ...
        private string GetRomanDayString(bool shorten = false)
        {
            ...
            if (this.DaysUntilPrincipalDay > 2)
            {
                _ = sb.Append($"{this.DaysUntilPrincipalDay.ToRomanNumerals()} ");
            }

            if (this.DaysUntilPrincipalDay == 0)
            {
        ...
        private int GetDaysUntil()
        {
                    ...
                    cMonth.Month == Months.Februarius ? 1 : 0), false))
            {
                ...
                return leapYear && cMonth.Month == Months.Februarius && day <= 24
                    ? daysBeforeKalendis
                    : leapYear && cMonth.Month == Months.Februarius && day > 24
                        ? daysBeforeKalendis + 1
                        : daysBeforeKalendis;
            }
